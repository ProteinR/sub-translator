...
Если вы хотите собрать проект в один исполняемый файл (например, `.exe` для Windows), чтобы запускать его без установленного Go, выполните команду:
```powershell
go build -o translator.exe
```
После этого вы сможете запускать программу просто двойным кликом по `translator.exe` или из терминала:
```powershell
.\translator.exe
```
